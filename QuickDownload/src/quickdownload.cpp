...
void QuickDownload::setRequestKeyAndValue(QString key, QString value){
...
void QuickDownload::start(QUrl url)
{
    ...
    shutdownNetworkReply();
    //configure the header to use the needed token. The key and value are QStrings, but the header takes a
    //QByteArray. Convert the QString to a QByteArray here.
    QNetworkRequest theRequest = QNetworkRequest(_url);
    QByteArray theKey;
    QByteArray theValue;
    theKey.append(_headerKey);
    theValue.append(_headerValue);
    theRequest.setRawHeader(theKey, theValue);
    _networkReply = qQuickDownloadMaster->networkAccessManager()->get(theRequest);
